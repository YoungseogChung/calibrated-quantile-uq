...
from losses import cali_loss, batch_cali_loss, qr_loss, batch_qr_loss, mod_cali_loss, batch_mod_cali_loss
from losses import *
...
def parse_args():
    ...
    parser.add_argument('--seed', type=int,
                        help='random seed')
    parser.add_argument('--data_dir', type=str, default='Synth_Datasets',
...
if __name__ == '__main__':
    ...
    model = vanilla_nn(input_size=1, output_size=dim_y,
                       hidden_size=64, num_layers=2)
    model = model.to(args.device)

    optimizer = torch.optim.Adam(model.parameters(),
                                 lr=args.lr, weight_decay=args.wd)
    loader = DataLoader(TensorDataset(y),
                        shuffle=False,
                        batch_size=num_pts)
                        # batch_size=args.bs)
    ...
    for q in q_list:
        ...
        print('Np quantile: {:.4f}, between {:.4f}, {:.4f}\n'.format(q_np, q_idx_1, q_idx_2))
