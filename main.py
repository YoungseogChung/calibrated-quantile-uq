...
if __name__ == "__main__":
    ...
    for d in DATA_NAMES:
        for s in tqdm.tqdm(SEEDS):
            ...
            if not args.boot:
                loader = DataLoader(
                    TensorDataset(x_tr, y_tr), shuffle=True, batch_size=args.bs
                )
            else:
                rand_idx_list = [
            ...
            reduced_va_q_preds = va_q_preds[
                :,
                get_q_idx(va_exp_props, 0.01) : get_q_idx(va_exp_props, 0.99)
                + 1,
            ]
            ...
            if "toy" in args.data_dir:
                ...
                for (x_arr, y_arr, pred_mat) in zip(
                    x_list, y_list, pred_mat_list
                ):
                ...
                pred_mat_list = [tr_q_preds, reduced_va_q_preds, te_q_preds]
                for (y_arr, y_order, pred_mat) in zip(
                    y_arr_list, y_order_list, pred_mat_list
                ):
