...
from losses import cali_loss, batch_cali_loss, qr_loss, batch_qr_loss, \
    ...
    interval_loss, batch_interval_loss, batch_diff_cali_loss, \
...
def parse_args():
    ...
    parser.add_argument('--sharp_all', type=int, default=0,
                        help='1 to penalize only widths that are over covered')

    # draw a sorted group batch every
    parser.add_argument('--g_every', dest='draw_group_every', type=int,
...
if __name__ == '__main__':
    ...
    for d in DATA_NAMES:
        for s in tqdm.tqdm(SEEDS):
            ...
            va_cali_score, va_sharp_score, va_obs_props, va_q_preds, _ = \
            ...
            np.save('{}_{}.npy'.format(args.loss, args.draw_group_every),
                    te_g_cali_scores)
